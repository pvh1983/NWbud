...
def proceess_raw_data(df):
    ...
    ofile = '../ouput/CaracteristiquesPEM_clean.csv'
    df.to_csv(ofile, index=None)
    print(f'New data was saved at {ofile} \n')


def func_cleanup_data(ifile, ifile_out, thres_depth_min, thres_depth_max, well_type, odir, cutoff_year):
    # Read input file
    ...
    print(f'\nFilted gwlevels were saved at {ofile} \n')
    f.close()
    return df


def func_plot_gwlevel_vs_year(df, odir, well_type):
...
def func_get_mean_gwlevel_given_year(df):
